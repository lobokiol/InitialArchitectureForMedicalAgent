...
- MCP 工具相关：
  - 病例查询工具：支持按患者姓名或患者ID查询病例信息。
  - 密码验证：查看病例需要密码验证（默认密码：888），防止隐私泄露。
...
- 文件：`app/domain/routing.py`
  - `route_after_decision(state)`：根据 `IntentResult` 决定后续节点：
    - 闲聊（非医学）→ `answer_generate`
    - 需要工具调用（病例查询）→ `tool_calling`
    - 有流程/症状意图 → `es_rag`
  - `route_after_tool_calling(state)`：工具调用后的路由：
    - 需要密码验证 → `answer_generate`（返回密码提示）
    - 密码已验证 → `es_rag`（继续检索流程）
...
- `decision_node`（意图识别）：`app/graph/nodes/decision.py`
  ...
  - 自动判断是否需要查询患者病例（通过正则匹配"病例"、"病历"、"患者XXX"等关键词）。

- `tool_calling_node`（工具调用/病例查询）：`app/graph/nodes/tool_calling.py`
  - 支持 MCP 工具调用：按患者姓名查询历史病例、按患者ID查询病例信息。
  - 密码验证机制：首次查询病例需要密码验证（默认密码 888），验证成功后当前会话无需再次验证。
  - 若需要密码验证，返回 `need_password_input=True`，前端提示用户输入密码。
  - 密码验证通过后，执行 MCP 查询，查询结果存入 `medical_docs`，继续后续流程。
...
- 本后端是一个**基于 LangGraph 的有状态医疗导诊助手**：
  ...
  - 通过意图识别将用户问题拆解为"症状问诊"、"流程问答"和"病例查询"，分别走 Milvus 向量检索、ES 流程检索和 MCP 工具调用。
  - 支持病例查询功能：按患者姓名或ID查询历史病例信息，带有密码验证机制保护隐私。
